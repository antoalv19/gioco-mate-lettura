...
import { byDifficulty, randomItem } from '../utils/game';
...
export function PracticeHub({ difficulty, lessons, score, best, onHome, onGoToGame }: { difficulty: Difficulty; lessons: PracticeLesson[]; score: number; best: number; onHome: () => void; onGoToGame: (m: Mode) => void }) {
  ...
  const choose = (selected: typeof areas[number]) => { setArea(selected.value); const matches = available.filter(item => item.area === selected.value); setLesson(matches.length ? randomItem(matches) : null); setState(null); };
  const retry = () => setState(null);
  const another = () => { if (!area) return; const matches = available.filter(item => item.area === area); setLesson(matches.length ? randomItem(matches) : null); setState(null); };
