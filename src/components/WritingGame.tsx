...
export function WritingGame({ difficulty, exercises, facts, score, best, onCorrect, onHome }: { difficulty: Difficulty; exercises: WritingExercise[]; facts: FunFact[]; score: number; best: number; onCorrect: () => void; onHome: () => void }) {
  ...
  const next = () => { setExercise(randomItem(available)); setKind(Math.random() > .5 ? 'listen' : 'complete'); setValue(''); setFeedback(null); setAttempts(0); setHint(0); };
  return <GameLayout score={score} best={best} onHome={onHome} title="Scrittura" icon="✏️"><section className="quiz-card writing-card"><p className="instruction">{kind === 'listen' ? 'Ascolta e scrivi la parola' : 'Completa la parola'}</p><div className="word-support">{difficulty !== 'difficile' && <span>{exercise.emoji}</span>}<strong>{kind === 'complete' ? exercise.completeWord.masked : '🔊'}</strong></div><ReadAloudButton text={exercise.word} label="Ascolta la parola" />
